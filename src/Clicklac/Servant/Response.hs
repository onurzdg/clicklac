...
encodeErr' :: (FailureMsg fm) => Int -> fm -> BL.ByteString
encodeErr' c fm = encode $
  ErrJSONRoot $ ErrJSON c (failMsg fm) [Err (failMsg fm) (failReason fm)]
...
err errCode msg errs = throwError $
 ...
 where
   ...
   errs' = foldr (\e a -> Err (failMsg e) (failReason e) : a) [] errs
