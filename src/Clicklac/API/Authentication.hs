...
import Clicklac.Email (validateEmail, Email(EmailU))
import Clicklac.OpFailure (FailureMsg(..))              
import Clicklac.Password (Password(UnValidatedPass), PWState(PWUnvalidated))  
...
instance ToSample Login where
  toSamples _ = singleSample $
    Login "foo@bar.com"  (UnValidatedPass "mypass")
